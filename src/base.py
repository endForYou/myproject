...
class Base:
    ...
    def enter_into_main_page_(self):
        ...
        # no_need_colleges = get_all_colleges_of_no_need(cursor, province="云南", data_type="enroll_plan")
    ...
    def get_one_page_enroll_plan_of_ben_ngk(self, colleges, no_need_colleges, province):
        ...
        for i in range(0, college_list_element_count):
            college_element = driver.find_elements(by=AppiumBy.ID, value="com.eagersoft.youzy.youzy:id/click_cl_parent")[i]
            if not is_element_present_by_element(college_element, "com.eagersoft.youzy.youzy:id/tv_name"):
                college_element.click()
            else:
                college = college_element.find_element(by=AppiumBy.ID, value="com.eagersoft.youzy.youzy:id/tv_name").text
    ...
    def get_one_page_major_score_line_of_ben_ogk(self,colleges, no_need_colleges, province):
        my_db = db.DataBase()
        cursor = my_db.get_cursor()
        driver = self.driver
        data_type = "enroll_plan"
    ...
    def get_one_page_major_score_line_of_ben_ngk(self, colleges, no_need_colleges, province):
        my_db = db.DataBase()
        cursor = my_db.get_cursor()
        driver = self.driver
